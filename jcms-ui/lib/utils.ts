...
export const getNavbarButtonsInfo = memoize(() => {
  const info = new Map<string, NavbarButtonProps[]>();

  for (const route of ROUTES) {
    if (info.has(route.itemGroup)) {
      info.get(route.itemGroup)!.push(route as NavbarButtonProps);
    }
    else {
      info.set(route.itemGroup, [ route as NavbarButtonProps ]);
    }
  }
