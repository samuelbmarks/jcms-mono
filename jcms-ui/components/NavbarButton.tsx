...
export function NavbarButton({ color, icon, title, href, onClick, expanded }: NavbarButtonProps) {
  ...
  const content = (
    ...
    >
      {Icon ? (
        <Center style={({ colors }) => ({ backgroundColor: colors[color][7] })} className={classes.iconWrapper} c="white">
          <Icon size={16} className={clsx(classes.icon, { [classes.expanded]: expanded })} />
        </Center>
      ) : (
        <Box style={({ colors }) => ({ borderColor: colors[color][7] })} className={classes.dotWrapper}>
